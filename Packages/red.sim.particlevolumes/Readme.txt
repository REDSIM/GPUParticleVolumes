...
This is a free open source Asset Package which includes one simple dust shader and an example prefab.
...
- To set actual particle count, you need to have a special particle mesh. (I provide mesh generator with paid asset packs, so by default only one example mesh is provided)
...
このアセットパッケージは、無料のオープンソースパッケージで、シンプルなダスト用シェーダー1つとサンプルのプレハブが含まれています。
